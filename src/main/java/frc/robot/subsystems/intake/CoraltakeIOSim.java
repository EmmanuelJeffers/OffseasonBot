...
public class CoraltakeIOSim implements CoraltakeIO {
    private final DCMotor gearbox = DCMotor.getKrakenX60Foc(1).withReduction(CoraltakeIOTalonFX.reduction);
    ...
    public CoraltakeIOSim() {
        simState = VecBuilder.fill(Coraltake.maxAngle, 0.0);
    }

    @Override
    public void updateInputs(CoraltakeIOInputs inputs) {
        if (!closedLoop) {
            controller.reset();
    ...
    private void update(double dt) {
        ...
        if (simState.get(0) >= Coraltake.maxAngle) {
        simState.set(0, 0, Coraltake.maxAngle);
        simState.set(1, 0, 0.0);
        }
    }
}